...
const sockets = []; // 가짜 데이터베이스에서 활용한 베열 생성
    ...
    socket.on("message", (msg) => { // message 이벤트 -> 콜백 함수는 msg를 전달 받음
        ...
        switch(message.type){ //message.type의 값에 따라 어떤 코드를 사용할 것인지 결정
            case "new_message": // 메시지의 내용인 payload를 전송
                ...
                // forEach 메소드: 배열의 각 요소에 차례로 접근해서 구문을 실행해주는 메소드
                break;
            case "nickname":
                socket["nickname"] = message.payload;
                // 스켓에 nickname 키 추가 -> 추가된 키에 payload 대입
                break;
        }
    });   
    // socket.send("hello!");
});
// handleConnection 함수를 선언한 다음에 on 메소드에 인지로 전달
// -> 같은 역할을 하는 익명함수를 만들어 on 메소드에 포함
// connection 이벤트가 발생했을 때 소켓을 받는다는 것을 직관적으로 표시
